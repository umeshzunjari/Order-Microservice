...
public class OrderController {
	...
	//Buyer view the details of particular buyer
		@GetMapping(value= "/orderdetails/{orderid}", produces = MediaType.APPLICATION_JSON_VALUE)
		public OrderDetailsDTO getOrder(@PathVariable Integer buyerid) {
			return orderService.getOrder(buyerid);
	 ...
	 * 
	 * @PostMapping
	 * (value="orders/placeorder",consumes=MediaType.APPLICATION_JSON_VALUE) public
	 * void placeOrder(@RequestBody Integer prodid) {
	 * logger.info("request for buy the product from cart {}", prodid); final String
	 * uri= cartServiceUrl; RestTemplate template=new RestTemplate(); CartDTO cart=
	 * template.getForObject(uri+prodid, CartDTO.class);
